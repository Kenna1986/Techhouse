...
// Thank you, Yoast de Valk, for much of this code.
	...
	//		$title = trim($title);
            ...
            $customTitle = tcfGetTaxonomyFieldValue($catId, 'title');
            if ($customTitle) {
                $title = str_replace('%custom_title%', $customTitle, $title);
            } else {
                $title = str_replace('%custom_title%', category_description($catId), $title);
            }
            //End custom title
            $title = $this->paged_title($title);
			$header = $this->replace_title($header, $title);
            ...
            // Add custom title
            $customTitle = tcfGetTaxonomyFieldValue(get_queried_object()->term_id, 'title');
            if ($customTitle) {
                $new_title = str_replace('%custom_title%', $customTitle, $new_title);
            } else {
                $new_title = str_replace('%custom_title%', $date, $new_title);
            }
            //End custom title
			$title = trim($new_title);
            $title = $this->paged_title($title);
